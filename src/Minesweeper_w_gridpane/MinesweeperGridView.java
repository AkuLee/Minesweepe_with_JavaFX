...
public class MinesweeperGridView extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        primaryStage.setTitle("Minesweeper_w_gridpane.MinesweeperGridView");
        primaryStage.show();
    }
    ...
    private void makeIntroScene() {
        ...
        submit.setOnAction(event -> { // On submit, recuperate all settings given by user and generate the game from it.
            ...
            controllerGrid = new ControllerGrid(this, length, height, nbMines);
    ...
    public void makeGrid() {
        ...
        for (int i = 0; i < height; i++) { // Y-axis
            for (int j = 0; j < length; j++) { // X-axis
                ...
                button.setOnAction((event) -> { // Recuperate (x,y) position and send to controllerGrid for logic changes.
                    String[] pos = button.getId().split("-");
                    controllerGrid.send(Integer.parseInt(pos[0]), Integer.parseInt(pos[1]));
                });
